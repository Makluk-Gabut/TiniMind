...
Yang TIDAK di-quantize (sengaja):
  - nn.Embedding (embed) dan lm_head — weight-tied satu sama lain
    (lihat model_v2.py: self.lm_head.weight = self.embed.weight).
    Quantize salah satu akan merusak tying. Dibiarkan FP16/FP32.
  - RMSNorm — parameter sangat sedikit, quantize tidak menghemat
    apa-apa dan bisa merusak stabilitas numerik norm.
  - RotaryEmbedding — tidak punya parameter trainable (hanya buffer).
